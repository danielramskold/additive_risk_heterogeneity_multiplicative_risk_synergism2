...
def separate_cause_sim(a_rel, b_rel, n, bg_freq1, bgfreq2, prevalence): # Additive O
...
def separate2_cause_sim(a_rel, b_rel, n, bg_freq1, bgfreq2, prevalence): # Additive R
    df = pandas.DataFrame()
...
def separate2_cause_sim_v2(a_rel, b_rel, n, bg_freq1, bgfreq2, prevalence): # Additive R
    df_a = pandas.DataFrame()
    df_b = pandas.DataFrame()
    case_control_split(df_a, 'outcome', prevalence, n//2)
    case_control_split(df_b, 'outcome', prevalence, n-n//2)
...
for prevalence in (0.5, 0.15, 0.05, 0.015, 0.005):
    for a_rel, b_rel, bg_freq1, bg_freq2 in zip(a_rel_arr, b_rel_arr, bg_freq_arr1, bg_freq_arr2):
        df = separate_cause_sim(a_rel, b_rel, n, bg_freq1, bg_freq2, prevalence)
        ...
        df = separate2_cause_sim(a_rel, b_rel, n, bg_freq1, bg_freq2, prevalence)
...
pyplot.savefig('plot%s_for_additive_spike_in_models_%s_%sprotective%s.pdf'%('_notched' if o.notch else '', 'correlation' if o.correlation else o.calc, o.protective, '_n=%i*%i'%tuple(o.n) if o.n != [1000, 1000000] else ''))