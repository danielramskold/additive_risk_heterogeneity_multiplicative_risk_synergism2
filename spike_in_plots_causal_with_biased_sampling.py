...
for prevalence in (0.5, 0.15, 0.05, 0.015, 0.005):
    for a_rel, b_rel, bg_freq1, bg_freq2 in zip(a_rel_arr, b_rel_arr, bg_freq_arr1, bg_freq_arr2):
        add_to_dict(log2_obs_to_exp, '%.3f\nsynth'%prevalence, calc_observed_expected(sample_ctrls(1, AND_logic_sim(a_rel, b_rel, n, bg_freq1, bg_freq2, prevalence)), o.calc=='RR'))
        add_to_dict(log2_obs_to_exp, '%.3f\nhetero'%prevalence, calc_observed_expected(sample_ctrls(1, OR_logic_sim(a_rel, b_rel, n, bg_freq1, bg_freq2, prevalence)), o.calc=='RR'))
        for threshold in [1,2,3,4,5]:
            df = sample_ctrls(1, threshold_three_of_five(a_rel, b_rel, n, bg_freq1, bg_freq2, prevalence, threshold))
...
pyplot.savefig('plot%s_for_biased_sampling_causal_spike_in_models_%s_%sprotective%s.pdf'%('_notched' if o.notch else '', 'correlation' if o.correlation else o.calc, o.protective, '_n=%i*%i'%tuple(o.n) if o.n != [1000, 1000000] else ''))
